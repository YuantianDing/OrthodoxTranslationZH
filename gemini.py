...
def translate(text: str, lang: str = 'cn') -> str:
    ...
    SYSTEM_PROMPT = (
        ...
        "采用俄罗斯东正教的传统翻译方式，尽量使用传统的翻译词汇。比如「Бог」翻译为「上帝」，但不用过与拘谨，可以为了诗性或神学上跟「人」对应而翻译为「神」；「Богородица」翻译为「圣母」，「Богоматеринство」翻译为「诞神女」；「Святой Дух」翻译为「圣灵」；「Сын Божий」翻译为「上帝之子」，「Богочеловек」翻译为「神人」。\n\n"
        "祷告时对上帝的称呼，称为「祢」，旧约和诗篇中对主上帝的称呼，采用「上主」。"
        "避免和合本式的「有福了/有祸了」，改用传统的「福哉/祸哉」。"
        "Отечника 翻译作教父言行录。\n\n"
