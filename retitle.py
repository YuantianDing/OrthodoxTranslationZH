...
assert level in ["heading1", "heading2", "heading3", "heading4", "placeholder"]
...
def process_block(block: Block, lang: str) -> str:
    if block['type'] in ['heading1', 'heading2', 'heading3', 'heading4']:
        ...
        is_placeholder = block['text'][lang] == "!!![To Be Retitled]"
        if block['type'] == level and 'original_title' not in list_get(block['children'], 0, {}).get('label', []) or is_placeholder:
            assert lang == 'ru'
